...
module.exports.run = async function ({ event, api, args, Currencies,Users,Threads }) {
    ...
    var turnbot = ["✌️","👊","✋"]
          ...
          if(coins <= 20) {
          api.sendMessage(`⚡️Số đặt cược của bạn phải lớn hơn hoặc bằng 20$`,event.threadID,event.messageID)
          }
          else{
          if(money < coins)  
          ...
          if (userturn == "kéo"||userturn == "búa"||userturn == "bao") {
            if (userturn == turnbot) {
              ...
              if (botturn == "👊") {
                ...
                Currencies.setData(event.senderID, options = {money: money +`${coins*5}`})
                return outMsg(`⚡️BẠN ĐÃ THẮNG\n\nUser : ${userturn}\nBot : ${botturn}\n\nBạn đã thắng và được cộng ${coins*5}$`)
