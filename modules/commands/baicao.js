...
module.exports.handleEvent = async ({ event, api, Users, Currencies }) => {
	...
	if (body.indexOf("Chia bài") == 0) {
		...
		return api.sendMessage("⚡ ️Pò chia bài rồi đó! tất cả mọi người đều có 2 lượt đổi bài nêú không thâý bài hãy kiểm tra lại tin nhắn chờ", threadID);
	}

	if (body.indexOf("Đổi bài") == 0) {
		if (values.chiabai != 1) return;
		var player = values.player.find(item => item.id == senderID);
		if (player.doibai == 0) return api.sendMessage("⚡️ Hết lượt đổi bài rồi nha", threadID, messageID);
		if (player.ready == true) return api.sendMessage("⚡️Bạn đã ready, hong thể đổi bài!", threadID, messageID);
	...
	if (body.indexOf("Ready") == 0) {
		...
		if (values.player.length == values.ready) {
			...
			return api.sendMessage(`⚡️Kết quả:\n\n ${ranking.join("\n")}\n\nRiêng người chơi đứng đầu nhận được ${values.rateBet * player.length}$ \n chơi cái này nhất ăn tất nha ,mà cờ bạc ít thôi cho Pò nhờ cái :<< `, threadID);
		}
		else return
	}
...
module.exports.run = async ({ api, event, args, Currencies }) => {
	...
	if (args[0] == "create") {
        ...
        if(money < args[1])  
        api.sendMessage(`⚡️Kiếm tiền rồi vào chơi nha ,baby cần ít nhất: ${args[1]}$`,event.threadID,event.messageID)
        else { 
